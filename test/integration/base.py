...
class DBTIntegrationTest(unittest.TestCase):
    ...
    def assertManyRelationsEqual(self, relations, default_schema=None, default_database=None):
        ...
        for relation in specs:
            # TODO: support bq and multi-schema queries, etc
            columns = self.get_relation_columns(relation)
            if first_columns is None:
                first_columns = columns
            else:
                self.assertEqual(first_columns, columns, '{} did not match {}'.format(str(specs[0]), str(relation)))
